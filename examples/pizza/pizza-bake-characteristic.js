...
PizzaBakeCharacteristic.prototype.onWriteRequest = function(data, offset, withoutResponse, callback) {
    if (offset) {
        ...
        console.log(`开始烤了，时间: ${temperature} -------onWriteRequest`)
        var self = this;
        this.pizza.once('ready', function(result) {
            console.log(`烤好了，时间: ${temperature} -------onWriteRequest  once 'ready' 还有个值 ${self.updateValueCallback}`)
            if (self.updateValueCallback) {
                var data = new Buffer(1);
                data.writeUInt8(result, 0);
                self.updateValueCallback(data);
                console.log(`updateValueCallback: ${data}`)

            }
        });
        this.pizza.bake(temperature);
        console.log(`烤好了，执行回调: ${Math.random()}`)
